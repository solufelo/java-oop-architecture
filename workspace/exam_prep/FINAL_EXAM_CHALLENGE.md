...
## Phase 2: Interfaces, Polymorphism & Covariant Returns (Weeks 8, 10)
...
3.  Create a `TeachingAssistant` class.
    ...
    -   **Covariant Return Type Challenge**: Override a method returning `Student` (e.g., `getSelf()`) to return `TeachingAssistant`.
...
3.  **The Queue:** Implement the `WaitQueue<T>` using the inner Node class.
    -   Methods: `enqueue(T item)`, `dequeue()`, `isEmpty()`.
